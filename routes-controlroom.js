...
const { queryControlRoom } = require('./db-controlroom');
...
// Initialize the control_room_pdfs table if it doesn't exist
const initializeTable = async () => {
  try {
    await queryControlRoom(`
      CREATE TABLE IF NOT EXISTS control_room_pdfs (
    ...
    await queryControlRoom(`
      CREATE INDEX IF NOT EXISTS idx_control_room_pdfs_date ON control_room_pdfs(pdf_date)
    `);

    console.log('ControlRoom: Table initialized successfully');
  } catch (error) {
    console.error('ControlRoom: Error initializing table:', error);
  }
};
...
router.post('/upload-pdf', upload.single('file'), async (req, res) => {
  try {
    ...
    const cardTypeMap = {
      "bulletin-report": "bulletinreport",
      "daily-report": "dailyreport",
      "pcc-floor-passport-report": "pccfloorpassportreport",
      "cctv-report": "cctvreport",
      "foot-patrolling-report": "footpatrollingreport",
      "record-accused-report": "recordaccusedreport"
    };

    const cardType = cardTypeMap[card_key] || card_key.replace(/-/g, '');
    const filename = `controlroom_${cardType}_${pdf_date}_${Date.now()}.pdf`;
    ...
    // Save to database
    const result = await queryControlRoom(
      `INSERT INTO control_room_pdfs (card_key, pdf_url, pdf_date, uploaded_by, updated_at)
    ...
    console.log('ControlRoom: PDF uploaded to Windows server and metadata saved for card:', card_key, 'date:', pdf_date);
    res.json({ message: 'PDF uploaded successfully', pdf_url: pdf_url, data: result.rows[0] });
  } catch (error) {
    console.error('ControlRoom: Error uploading PDF:', error);
...
router.get('/pdfs', async (req, res) => {
  try {
    ...
    const result = await queryControlRoom(
      'SELECT card_key, pdf_url, pdf_date FROM control_room_pdfs WHERE pdf_date = $1',
      [date]
    );

    res.json(result.rows);
  } catch (error) {
    console.error('ControlRoom: Error fetching PDFs:', error);
...
router.post('/pdfs', async (req, res) => {
  try {
    ...
    console.log('ControlRoom: PDF data upserted successfully for card:', card_key, 'date:', pdf_date);
    res.json({ message: 'PDF data saved successfully', data: result.rows[0] });
  } catch (error) {
    console.error('ControlRoom: Error upserting PDF data:', error);
...
router.delete('/pdfs', async (req, res) => {
  try {
    ...
    const pdfRecord = await queryControlRoom(
      'SELECT pdf_url FROM control_room_pdfs WHERE pdf_date = $1 AND card_key = $2',
      [date, card_key]
    );
    ...
    if (fs.existsSync(filePath)) {
      ...
      console.log('ControlRoom: File deleted from storage:', filePath);
    }

    // Delete from database
    const result = await queryControlRoom(
      'DELETE FROM control_room_pdfs WHERE pdf_date = $1 AND card_key = $2 RETURNING *',
      [date, card_key]
    );

    console.log('ControlRoom: PDF deleted successfully for card:', card_key, 'date:', date);
    res.json({ message: 'PDF deleted successfully', deleted: result.rows[0] });
  } catch (error) {
    console.error('ControlRoom: Error deleting PDF:', error);
...
router.get('/latest-date', async (req, res) => {
  try {
    const result = await queryControlRoom(
      'SELECT pdf_date FROM control_room_pdfs ORDER BY pdf_date DESC LIMIT 1'
    );
    ...
    console.error('ControlRoom: Error fetching latest date:', error);
...
router.get('/dates', async (req, res) => {
  try {
    const result = await queryControlRoom(
      'SELECT DISTINCT pdf_date FROM control_room_pdfs ORDER BY pdf_date DESC'
    );

    res.json(result.rows);
  } catch (error) {
    console.error('ControlRoom: Error fetching dates:', error);
...
router.get('/health', async (req, res) => {
  try {
    await queryControlRoom('SELECT 1');
    res.json({ status: 'healthy', message: 'Control Room database connection is working' });
  } catch (error) {
    res.status(500).json({ status: 'unhealthy', message: 'Control Room database connection failed', details: error.message });
  }
});
